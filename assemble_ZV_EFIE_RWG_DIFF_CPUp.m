function [Z, Vrhs] = assemble_ZV_EFIE_RWG_DIFF_CPUp(mesh, rwg, k, omega, mu, eps, exc, opts)

Ne = rwg.Ne;
Z  = complex(zeros(Ne,Ne));

% Quadrature ve geometri precompute (bir kere)
[qp, qw] = tri_quad_rule(opts.quad_order);
tri      = precompute_tri_geom(mesh);

% ===== Z montajı (PARFOR) =====
% Not: Parallel Computing Toolbox gerekir.
parfor m = 1:Ne
    tm = [rwg.plusTri(m), rwg.minusTri(m)];

    rowZ = complex(zeros(1,Ne));  % bu m satırı
    ...
    for n = 1:Ne
        ...
        for im = 1:2
            ...
            for in = 1:2
                ...
                if t1 == t2
                    [Ivec, Isca] = rwg_pair_integrals_singular( ...
                        mesh, tri, rwg, m, n, t1, t2, qp, qw, k, opts);
                elseif triangles_are_near(tri, t1, t2, opts.near_sing_eps)
                    [Ivec, Isca] = rwg_pair_integrals_near_singular( ...
                        mesh, tri, rwg, m, n, t1, t2, qp, qw, k, opts);
                else
                    [Ivec, Isca] = rwg_pair_integrals_regular( ...
                        mesh, tri, rwg, m, n, t1, t2, qp, qw, k);
                end
    ...
    Z(m,:) = rowZ; % parfor için doğru/izinli yazım
end

% ===== RHS: diferansiyel Gaussian edge-strip =====
